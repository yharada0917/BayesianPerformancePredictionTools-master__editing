...
def mesh_3D_data_from_stacked3obj(xp_val_l, xm_val_l, y_predict_l):
    na_fill_obj = ''
    colnom_list = sorted(list(set(xp_val_l)))
    ixnom_list = sorted(list(set(xm_val_l)))
    d = dict()
    for x,y,z in zip(xp_val_l, xm_val_l, y_predict_l):
        d.update({(x,y):z})
    mesh_x = list() ; mesh_y = list() ; listed_lis = list()
    for colnom in colnom_list:
        lis_x = list()
        lis = list()
        for ixnom in ixnom_list :
            lis_x.append(colnom)
            try:
            ...
            mesh_x.append(lis_x)
            mesh_y.append(ixnom_list)
            listed_lis.append(lis)
    return(mesh_x, mesh_y, listed_lis)
    # original ; bayesian_5PM_parameters_04_fakedata.py
